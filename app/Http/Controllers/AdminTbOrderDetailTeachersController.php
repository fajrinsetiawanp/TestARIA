<?php namespace App\Http\Controllers;
	...
	class AdminTbOrderDetailTeachersController extends \crocodicstudio\crudbooster\controllers\CBController {
	    ...
	    public function cbInit() {
			...
			$this->table = "tb_order_detail_teachers";
			...
			$this->col[] = ["label"=>"Reward Percent","name"=>"reward_percent"];
			$this->col[] = ["label"=>"Reward Total","name"=>"reward_total","callback_php"=>"number_format([reward_total])"];
			...
			$this->form[] = ['label'=>'Harga Satuan','name'=>'harga_satuan','type'=>'hidden','validation'=>'required|min:1|max:255','width'=>'col-sm-10'];
			$this->form[] = ['label'=>'Diskon','name'=>'diskon','type'=>'hidden','validation'=>'required|integer|min:0','width'=>'col-sm-10'];
			...
			$this->form[] = ["label"=>"Diskon","name"=>"diskon","type"=>"hidden"];
			//$this->form[] = ["label"=>"Total","name"=>"total","type"=>"text","required"=>TRUE,"validation"=>"required|min:1|max:255"];
			$this->form[] = ["label"=>"Reward Percent","name"=>"reward_percent","type"=>"hidden"];
			//$this->form[] = ["label"=>"Reward Point","name"=>"reward_point","type"=>"text","required"=>TRUE,"validation"=>"required|min:1|max:255"];
			$this->form[] = ["label"=>"Reward Total","name"=>"reward_total","type"=>"hidden"];
			# OLD END FORM
	    ...
	    */
	    public function hook_before_add(&$postdata) {        
	        //Your code here
	        $harga_produk = TbHargaProduk::where('id_produk', $postdata['id_produk'])->where('jenis_harga','Retail')->first();
			$postdata['harga_satuan'] = $harga_produk->harga;

			$total = $postdata['harga_satuan'] * $postdata['qty'];
	        $diskon = $total * ($harga_produk->diskon/100);

	        $postdata['diskon'] = $harga_produk->diskon;
	        $postdata['total'] = $total - $diskon;

	        $postdata['reward_percent'] = $harga_produk->reward_percent;
	        $postdata['reward_total'] = $postdata['total'] * ($postdata['reward_percent']/100);
